...
%prep
%setup -q
LIBPATH="%{aster_libs}scotch-5.1.11/lib  %{aster_libs}metis-4.0.3/lib/ %{aster_libs}OpenBLAS/lib /usr/lib64/" INCLUDES="%{aster_libs}scotch-5.1.11/include/ %{aster_libs}metis-4.0.3/include %{aster_libs}OpenBLAS/include" ./waf configure --maths-libs="openblas" --embed-maths --install-tests --prefix=%{buildroot}%{_prefix}
