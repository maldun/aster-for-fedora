...
%setup -q
cp %SOURCE1 wafcfg/
export PATH=%{mpidir}/bin:%{aster_libs}/mfront-%{mfront_version}/bin:$PATH; export LD_LIBRARY_PATH=%{mpidir}/include:%{aster_libs}/mfront-%{mfront_version}/lib:$LD_LIBRARY_PATH; ./waf configure --use-config-dir=wafcfg --use-config=%{config_file} --destdir=%{buildroot} --prefix=%{_prefix}
