...
QA_SKIP_BUILD_ROOT=1 rpmbuild -ba codeaster-stable-mumps-openmpi.spec --define "version $MUMPS_STABLE_VER" --define "scalapack_version $SCALAPACK" --define "aster_root $ASTER_BASE" --define "aster_libs $ASTER_LIBS" --define "openblas_lib $OPENBLAS_LIB" --define "openblas_inc $OPENBLAS_INC" --define "scotch_version $SCOTCH_VER" --define "metis_version $METIS_VER" --define "libdir $SYSTEM_LIBS" --define "mpidir $MPI_DIR"
